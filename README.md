# @shapeshift-labs/frontier-dataflow

Serializable incremental dataflow and materialized-view graphs for Frontier apps.

`frontier-dataflow` makes derived state explicit: dependency DAGs, source selectors, filters, groupings, aggregates, joins, materialized output paths, stale-node plans, recompute budgets, patches, and provenance records. It answers not just that a dashboard changed, but which source paths contributed to the output.
...
```sh
npm install @shapeshift-labs/frontier-dataflow
```
...
} from '@shapeshift-labs/frontier-dataflow';

const graph = compileDataflow(createDataflowGraph({
  ...
  nodes: [
    ...
    { id: 'customerTotals', input: 'ordersByCustomer', aggregate: { op: 'sum', field: 'total' }, outputPath: '/views/customerTotals' },
    { id: 'recentInvoices', sourcePath: '/entities/recentInvoices' },
    { id: 'dashboard', inputs: ['customerTotals', 'recentInvoices'], join: { fields: ['total', 'invoice'] }, outputPath: '/dashboard/revenue' }
  ]
}));

const plan = planDataflowRecompute(graph, {
  patches: [{ op: 'set', path: '/entities/orders/o1/status', value: 'paid' }]
});

const materialized = materializeDataflow(graph, state);
const why = explainDataflowChange(materialized, { path: '/dashboard/revenue/c1' });
```

## Surface

- `createDataflowGraph`, `defineDataflowNode`, `compileDataflow`, `validateDataflowGraph`, and `queryDataflowGraph` describe derived-data DAGs.
- `planDataflowRecompute` reports stale nodes, stale source paths, stale output paths, touched rows, and recompute budget from patches or changed paths.
- `materializeDataflow` evaluates source/filter/groupBy/aggregate/join nodes and emits materialized output patches.
- `explainDataflowChange` returns provenance records for a materialized output path, including source paths and input node ids.
- `createDataflowRegistryGraph` and `traceDataflowImpact` make dataflow graphs queryable with the rest of the Frontier registry surfaces.
- `encodeDataflowJsonl`, `decodeDataflowJsonl`, `redactDataflowValue`, and `createDataflowProof` support replay, redaction, and evidence bundles.

## Frontier View Boundary

`@shapeshift-labs/frontier-view` maps data to renderer-neutral view frames, visual channels, validation frames, and action bindings. It should consume dataflow outputs when a UI wants a dashboard or projection, but it should not own the dependency DAG, stale-row computation, recompute budget, or provenance.

`@shapeshift-labs/frontier-state` and `@shapeshift-labs/frontier-state-cache` already maintain local views and query indexes. `frontier-dataflow` sits above those as an inspectable derived-data contract, with structural adapters for state, cache, scheduler, trace, policy, workflow, worker, and view packages.
...
The benchmark writes `benchmarks/results/frontier-dataflow-package-bench-latest.json` when run from the monorepo. These are Frontier-only package measurements for graph creation, compilation, recompute planning, materialization, provenance explanation, impact tracing, registry graph output, JSONL, and proof hashing.
